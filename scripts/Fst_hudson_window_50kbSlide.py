...
python Fst_hudson_window_50kbSlide.py	--pop1 < Count file from population 1 needing to go into analysis > 
								...
								--chromSizefile < The chromosome size file for your species, chromosome with length ? 
pop1 == Count file from population 1 needing to go into analysis
pop2 == Count file from population 2 needing to go into analysis
chromSizefile == Chromosome name is column 1, chromosome length (in bp) is column 2
out == Output file stem name
"""
...
parser.add_option('--chromSizefile',dest='chromSizefile', help = 'Chromosome size file')
...
# Example format for the input count files are as follows: 
# chrom, position, allele total (below are all diploid), number of haplotypes (below represent 82 samples * 2 = 64 haplotypes),
#      Count of allele 1 (nucleotide:count),   Count of allele 2 (nucleotide:count)
 
"""
chr1    32998   2       164     C:164   T:0 
chr1    33084   2       164     C:164   T:0  
chr1    33288   2       164     C:141   T:23 
...
###USER DEFINED COUNT FILE
dogCountFile = options.pop1 #Counts generated by VCF tools for dogs only
wolfCountFile = options.pop2 #Counts generated by VCF tools for wolves only

#Define outfile
fstOutfile = options.out + '_Hudson_Fst_200kbWindow_50kbSlide.txt' #Write out the new FST data here
fstOutFile = open(fstOutfile, 'w')

#Define header to write out to outfile
...
###USER DEFINED CHROMOSOME SIZE FILE
chromSizefile = options.chromSizefile
...
for line in chromSizeFile:
	# ***** USERS MAY NEED TO CHANGE THIS *****
	# HERE WE IGNORE CHROMOSOME UNKNOWN AND MITOCHONDRIA!!
	if "chrUn" in line: #ignore chromosome unknowns 
		continue
...
for chrom in chromlist:
	...
	for i in range(1,130000000,50000):  #figure out the head/tail index
		...
		for k in range(head_idx, tail_ind_new+1): #This is going to calculate the F,num,denom at each site in the window
			...
			num = temp[1] #calculates the numerator for Fst calculation (below)
			denom = temp[2] #calculates the denominator for Fst calculation (below)
		...
		if len(fList) == 0:
			continue
		AofR = float(sum(fList))/len(fList) #average of ratios
		...
		RofA = float(meanNum)/meanDenom #ratio of averages
		########################
